...
public static class Menu // Класс, который мы будем использовать для взаимодействия с пользователем.
{
    ...
    private static void WriteMenu(List<Apartments> list)
    {
        ...
        while (writeFlag == true)
        {
            ...
            {
                case 1:
                    Console.Write("\nУкажите путь к файлу, который нужно перезаписать: ");
                    ...
                    Console.WriteLine("3) Выйти из меню.");
                    Console.Write("\nВведите опцию: ");
                    int flag; // Переменная, в которую будет считываться номер выбранной опции.
                    while (!int.TryParse(Console.ReadLine(), out flag) && flag < 0 && flag > 3) // Проверка на корректность введенной опции.
                    {
                        Console.WriteLine("Ошибка, введены неправильные значения."); // Сообщаем пользователю об ошибке.
                    }
                    ...
                    switch (flag)
                    {
                        case 1: // Пользователь выбрал перезаписать в старый файл стандартный поток ввода-вывода.
                            JsonParser.WriteJson(list, 1, 1);
                            break;
                        case 2: // Пользователь выбрал перезаписать в старый файл файловый ввод-вывод.
                            JsonParser.WriteJson(list, 1, 2);
                            break;
                    }       
                    break;
                case 2:
